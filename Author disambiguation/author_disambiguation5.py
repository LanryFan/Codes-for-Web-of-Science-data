...
Searching co_authors of both two matching author.

@author: Lanry Fan
"""

import pymysql
import same_coauthor

db=pymysql.connect('localhost','user','psw','dbname')
#Step-1:Initial [aid_raw4] use [aid_raw3]
cursor_init=db.cursor()
sql_init='update author_init set aid_raw8=aid_raw7'
try:
    print('正在初始化[aid_raw8]...')
...
try:
    ...
    for row in rs1:
        ...
        sql_disab='select xuhao,aid_raw8,fname,mname,fname_initial,mname_initial\
        ...
        try:
            ...
            #Step-4:Author disambiguation use the co_author
                            ...
                            if data_disab[i][2]!='' and data_disab[j][2]!='':
                                ...
                                if data_disab[i][3]!='' and data_disab[j][3]!='':
                                    ...
                                    if data_disab[i][2]==data_disab[j][2] and data_disab[i][3]==data_disab[j][3]:
                                        #IF two author's have at least one same co_author,decide them as one author                                    
                                        mycoauthor=same_coauthor.co_author()
                                        tag=mycoauthor.has_coauthor(data_disab[i][6],data_disab[j][6])                                                                        
                                        if tag:
                                            aid1=data_disab[i][1]
                                            aid2=data_disab[j][1]
...
try:
    for row in data:
        ...
        sql_update='update author_init set aid_raw8=%d\
